import { marchingCubes } from '../modules/mc-webgpu/mc-webgpu.js';
...
async function main() {
    //const volumeData = new Uint32Array(await requestFile("https://www.dl.dropboxusercontent.com/s/3ykigaiym8uiwbp/aneurism_256x256x256_uint8.raw?dl=1", 'arraybuffer'));
    const volumeData = new Uint32Array(await requestFile("https://www.dl.dropboxusercontent.com/s/ic0mik3qv4vqacm/foot_256x256x256_uint8.raw?dl=1", 'arraybuffer'));
	const meshData = await marchingCubes(volumeData);
    ...
    // Shaders
    const vertShaderCode = await requestFile("src/shaders/vertex-shader.vert", 'text');
    const fragShaderCode = await requestFile("src/shaders/fragment-shader.frag", 'text');
    shader = new Shader (gl, vertShaderCode, fragShaderCode);
    shader.use(gl);
