...
const float voxelSize[3] = { 0.1201914, 0.1201914, 0.2999309 };

unsigned char * jfa3(unsigned char *volume, int width, int height, int depth, int start, int stop) {
    int d = stop - start;
    unsigned char target = 3;

    std::vector<bool> mask(width * height * depth, false);
    ...
    // First, fill the half-resolution mask with the seed sites.
    unsigned char site;
    for (int k = 0; k < depth; k++) {
        for (int j = 0; j < height; j++) {
            for (int i = 0; i < width; i++) {
                site = volume[i + j * width + k * width * height];
                if (site == target) {
                    mask[i + j * width + k * width * height] = true;
                }
            }
        }
    }
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            for (k = start; k < depth; ++k) {
                if (mask[i + j * width + k * width*height]) {
            ...
            for (int i = 0; i < width; ++i) {
                ...
                if (up_zdist[j*width+i] < 0) {
                    ...
                    for (k = slice; k < depth; ++k) {
                        if (mask[(k * height + j) * width + i]) up_zdist[j * width + i] = k - slice;
                    }  
                }
